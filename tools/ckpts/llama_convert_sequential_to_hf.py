...
def create_config(neox_config):
    ...
    pad_token = tokenizer.pad_id  # Set in HFLlamaTokenizer but not used
    
    # try:  # HFLlamaTokenizer raises NotImplementedError
    ...
    use_tied_lns = get_key(neox_config, "gpt-j-tied", False) # Llama에서 False
    ...
    multiple_of = 256
    ff_dim = int(2 * get_key(neox_config, "hidden-size") * 4 / 3)
    ...
    hf_config = LlamaConfig(
        ...
        # layer_norm_eps=get_key(neox_config, "layernorm-epsilon", 1e-5), # config not in Llama
        use_cache=True,
        bos_token_id=tokenizer.bos_id,
        eos_token_id=tokenizer.eod_id,
        tie_word_embeddings=(not get_key(
            neox_config, "no-weight-tying", False)), # transformers PretrainedConfig defalt is True. transformers Llama defaults to False
        # use_parallel_residual=get_key(neox_config, "gpt-j-residual", False), # config not in Llama
        rms_norm_eps=get_key(neox_config, "rms_norm_epsilon", 1e-5),
        rope_theta=get_key(neox_config, "rotary_emb_base", 10000.0),
    )
    
    """configs in LlamaConfig but not in GiLlama
    - num_key_value_heads = default num_attention_heads
    - pretraining_tp = default 1. Experimental feature. Tensor parallelism rank used during pretraining.
    - rope_theta = default 10000.0. The base period of the RoPE embeddings.
    - rope_scaling(Dict) = ??
...
def convert(input_checkpoint_path, loaded_config, output_checkpoint_path):
    ...
    print("debug: loaded all model states.")
    # debug
    # config_path = os.path.join(os.getcwd(), "GiLlama_HFconfig.txt")
    # tensor_path = os.path.join(os.getcwd(), "GiLlama_tensors.pt")
    # with open(config_path, "w", encoding="utf-8") as file:
    #     file.write(str(hf_config) + "\n\n")
    #     file.write(str(hf_model) + "\n\n")
    # torch.save(loaded_tp_ranks, tensor_path)
    # print("GiLlama HF config saved at: ", config_path)
    # print("GiLlama tensors saved at: ", tensor_path)
    # exit(1)

    ### Embedding layer ###
    # TODO Need to change GPT-NeoX layer name into corresponding Llama layer
    ...
    ), f"ERROR: calculated vocab size {hf_config.vocab_size} != embed param size {hf_model.model.embed_tokens.shape[0]}"
    ### End Embedding Layer ###
    ...
    for layer_i in tqdm(range(get_key(loaded_config, "num-layers"))):
        ...
        state_dict["self_attn.o_proj.weight"] = torch.cat(
            get_state(loaded_tp_ranks, "attention.dense.weight", layer_i+2), dim=1
        )
        state_dict["mlp.down_proj.weight"] = torch.cat(
            get_state(loaded_tp_ranks, "mlp.w2.weight", layer_i+2), dim=1
        )
        
        state_dict["input_layernorm.weight"] = sum(
            get_state(loaded_tp_ranks, "input_layernorm.scale", layer_i+2)
        ) / len(loaded_tp_ranks)
        state_dict["post_attention_layernorm.weight"] = sum(
            get_state(loaded_tp_ranks, "post_attention_layernorm.scale", layer_i+2)
        ) / len(loaded_tp_ranks)

        state_dict["mlp.gate_proj.weight"] = torch.cat(
                get_state(loaded_tp_ranks, "mlp.w1.weight", layer_i + 2), dim=0
            )
        state_dict["mlp.up_proj.weight"] = torch.cat(
                get_state(loaded_tp_ranks, "mlp.w3.weight", layer_i + 2), dim=0
            )
        
        qkv = torch.cat(
            get_state(loaded_tp_ranks, "attention.query_key_value.weight", layer_i+2), dim=0
        )
        
        # TODO check if below is correct
        h = hf_config.hidden_size
        state_dict["self_attn.q_proj.weight"] = qkv[:h, :]
        state_dict["self_attn.k_proj.weight"] = qkv[h:2*h, :]
        state_dict["self_attn.v_proj.weight"] = qkv[2*h:, :]
        ...
        # Just take one
        state_dict["self_attn.rotary_emb.inv_freq"] = get_state(
